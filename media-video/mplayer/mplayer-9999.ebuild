...
ftp ggi gsm +iconv ipv6 jack joystick jpeg jpeg2k kernel_linux ladspa
...
radio +rar +real +rtc rtmp samba selinux +shm sdl skin +speex cpu_flags_x86_sse cpu_flags_x86_sse2 cpu_flags_x86_ssse3
tga +theora +tremor +truetype +toolame +twolame +unicode v4l vcd vdpau vidix
+vorbis win32codecs +X x86-fbsd +x264 xanim xinerama +xscreensaver +xv +xvid xvmc yuv4mpeg zoran"
...
# We remove the ffmpeg depend as mplayer will download it and be staticaly linked
...
pkg_setup() {
	...
	elog "It follow upstream recommendation regarding the libraries,"
	elog "which make it fully unsuported by Gentoo."
	elog "Report ebuild issues to the pro-audio overlay."
	elog
...
#	touch ffmpeg/mp_auto_pull
	...
	# Pulls an outdated libopenjpeg, ffmpeg provides better support fot it
